...
public class BulletManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ClientScene.RegisterSpawnHandler(assetId, SpawnBullet, DestroyBullet);
    }
    ...
    public void DestroyBullet(GameObject spawned)
    {
        Destroy(spawned);
    }
}
...
public class Player : NetworkBehaviour
{
    ...
    private void CmdFire()
    {
        ...
        StartCoroutine(Destroy(bullet, 3.0f));
    }

    private IEnumerator Destroy(GameObject go, float timer)
    {
        yield return new WaitForSeconds(timer);
        bulletManager.DestroyBullet(go);
